...
sLog     = "C:\inetpub\vhosts\cbm.am.gov.br\itsm\logs\backend-node.log"
...
Response.Write "Encerrando processo anterior na porta 4000..." & vbCrLf
oShell.Run "cmd /c for /f ""tokens=5"" %a in ('netstat -ano ^| findstr :4000 ^| findstr LISTENING') do taskkill /PID %a /F", 0, True
Response.Write "<span class='ok'>OK</span> - Porta 4000 liberada" & vbCrLf & vbCrLf
...
"  <RegistrationInfo><Description>CBMAM ITSM Backend Node.js porta 4000</Description></RegistrationInfo>" & vbCrLf & _
...
Dim nCheck
nCheck = oShell.Run("cmd /c netstat -ano | findstr :4000 | findstr LISTENING > NUL 2>&1", 0, True)
If nCheck = 0 Then
    Response.Write "<span class='ok'>checkmark SUCESSO! Node.js esta ouvindo na porta 4000!</span>" & vbCrLf
Else
